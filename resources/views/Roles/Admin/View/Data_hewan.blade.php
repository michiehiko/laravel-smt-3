...
<main class="flex-grow container mx-auto px-4 py-12">
    ...
    <div class="mb-8">
        <div class="bg-gradient-to-r from-amber-400 via-amber-500 to-amber-400 rounded-t-2xl shadow-xl p-6">
            ...
            <p class="text-blue-800 mt-2 ml-14">Data hewan peliharaan yang terdaftar</p>
        </div>
    </div>

    <!-- Tombol Tambah -->
    <div class="mb-6">
        <a href="{{ route('create_hewan') }}" 
            ...
            <table class="w-full">
                <thead class="bg-gradient-to-r from-blue-900 to-blue-800 text-white">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left font-bold text-lg">Nama Hewan</th>
                        <th class="px-6 py-4 text-left font-bold text-lg">Jenis Hewan</th>
                        <th class="px-6 py-4 text-left font-bold text-lg">Ras</th>
                        <th class="px-6 py-4 text-left font-bold text-lg">Pemilik</th>
                        <th class="px-6 py-4 text-left font-bold text-lg">Jenis Kelamin</th>
                        <th class="px-6 py-4 text-left font-bold text-lg">Tanggal Lahir</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-blue-50 transition-colors duration-200">
                        ...
                        <td class="px-6 py-5 font-medium text-gray-800">{{ $p->nama }}</td>
                        <td class="px-6 py-5 text-gray-700">{{ $p->rasHewan->jenisHewan->nama_jenis_hewan ?? '-' }}</td>
                        <td class="px-6 py-5 text-gray-700">{{ $p->rasHewan->nama_ras ?? '-' }}</td>
                        <td class="px-6 py-5 text-gray-700">{{ $p->pemilik->user->nama ?? '-' }}</td>
                        ...
                        <td class="px-6 py-5">
                            ...
                            @elseif($p->jenis_kelamin == 'B')
                                <span class="inline-flex items-center gap-2 bg-pink-100 text-pink-700 px-3 py-1 rounded-full text-sm font-semibold">
                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clip-rule="evenodd"></path>
                                    </svg>
                                    Betina
                        ...
                        <td class="px-6 py-5 text-gray-700">{{ $p->tanggal_lahir }}</td>

                        <td class="px-6 py-5 text-center">
                            <div class="flex items-center justify-center gap-2">

                                {{-- Edit --}}
                                 <a href="{{ route('edit_hewan', $p->idpet) }}" 
                                ...
                                {{-- Delete --}}
                                <form action="{{ route('hapus_hewan', $p->idpet) }}" method="POST" 
                                    onsubmit="return confirm('Hapus hewan {{ $p->nama }}?')"
                                    class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-xl font-bold text-gray-700 mb-2">Belum ada data hewan</p>
                                <p class="text-gray-500">Silahkan tambahkan data hewan baru</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
